...
def hangman():
    ...
    while len(word_letters) > 0 and lives > 0:
        #letter used
        #''.join(['a', 'b', 'cd']) --> 'a b cd'
        print('You have', lives,'lives left andyou have used these letters: ', ''.join(used_letters))
        ...
        if user_letter in alphabet - used_letters:
            ...
            if user_letter in word_letters:
                ...
                print('\nYous letter, ' , user_letter, 'is not in the word')
        elif user_letter in used_letters:
            print('\nYou have already used that character. Please try again.')
        else:
            print('Invalid Character. Please try again')

#gets here when len(word_lettes) == 0 or when lives == 0
    if lives == 0:
        print(lives_visual_dict[lives])
        print('You died,  sorry. Tthe word waa', word)
    else:
